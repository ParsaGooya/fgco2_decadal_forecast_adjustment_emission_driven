...
def predict(fct:xr.DataArray , observation:xr.DataArray , params, lead_years, model_dir,  test_years, model_year = None, ensemble_list = None, ensemble_mode = 'Mean', btstrp_it = 200, save=True):
    ...
    if params['version_patterns'] == 3:
        ...
        forecast_preprocessing_steps_patterns =  [('anomalies', AnomaliesScaler_v1(axis=0)),  ('standardize', Standardizer(axis = (0,1))) ]
        ...
        forecast_preprocessing_steps_patterns =  [('anomalies', AnomaliesScaler_v2(axis=0)),  ('standardize', Standardizer(axis = (0,1))) ]
        observations_preprocessing_steps_patterns = [('anomalies', AnomaliesScaler_v2(axis=0)) ]

    else:
        
        forecast_preprocessing_steps_patterns =  [ ('standardize', Standardizer(axis = (0,1))) ]
    ...
    if ensemble_mode == 'Mean': ##
        ...
        forecast_preprocessing_steps_patterns =  [ ('standardize', Standardizer(axis = (0,1,2))) ]
    ...
    if params["model_mean"] is not None:
        test_set_mean = XArrayDataset(ds_test_mean, xr.ones_like(ds_test_mean).rename({'lead_time':'month'}), extra_predictors = extra_predictors, lead_time=None, time_features=time_features_mean,ensemble_features =ensemble_features,  in_memory=False, aligned = True, year_max = year_max)
...
if __name__ == "__main__":
    ...
    out_dir_x  = f'/space/hall5/sitestore/eccc/crd/ccrn/users/rpg002/output/fgco2_ems/SOM-FFN/results/Autoencoder/run_set_3'
    out_dir    = f'{out_dir_x}/N35_v03_decoupled_LNone_arch2_batch64_e60_realistic_emissions_after_2016' 
    ...
    fct_ = xr.open_mfdataset(str(Path(data_dir_forecast, "*.nc")), combine='nested', concat_dim='year').load()
    ...
    if bootstrap:
        ...
        fct_real = xr.open_mfdataset(str(Path('/space/hall5/sitestore/eccc/crd/ccrn/users/rsa001/big_store/AI/FGCO2/data/forecasts/canesm5-ems-gcb', "*.nc")), combine='nested', concat_dim='year').load() 
        for yr in fct_real.year.values:
                fct = fct_.where(fct_.year < yr, drop = True)
                fct = xr.concat([fct, fct_real.sel(year= yr)], dim = 'year')
                test_years = [yr] 
    ...
    #     print( f'running for ensemble member {ens}')

    #     params = extract_params(out_dir + f'/L{ens}')
    #     print(f'loaded configuration: \n')
    #     for key, values in params.items():
    #         print(f'{key} : {values} \n')
    ...
    #     predict(fct, observation, params, lead_years, out_dir + f'/L{ens}',  test_years, ensemble_mode='Mean',  save=True)
