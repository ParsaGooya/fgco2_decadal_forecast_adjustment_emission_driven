...
from data_locations import LOC_FORECASTS_fgco2, LOC_OBSERVATIONS_fgco2_v2023, LOC_OBSERVATIONS_fgco2_v2022
import gc
...
def run_training(params, n_years, lead_years, n_runs=1, results_dir=None, numpy_seed=None, torch_seed=None, save = False):
    ...
    for run in range(n_runs):
        ...
        for y_idx, test_year in enumerate(test_years):
            ...
            train_set = XArrayDataset(ds_train, obs_train, mask=train_mask, extra_predictors= extra_predictors,lead_time_mask = params['lead_time_mask'], in_memory=True, lead_time=lead_time, time_features=time_features, aligned = True, year_max = year_max, VAE = params['VAE']) 
            ...
            if params['VAE'] is None:
                ...
                # else:
                #     if type(reg_scale) == dict:
                #         criterion = WeightedMSESignLossKLD(weights=weights, device=device, hyperparam=hyperparam, reduction='mean', loss_area=loss_region_indices, scale=reg_scale[test_year], min_val=0, max_val=None)
                #     else:
            ...
            for epoch in tqdm.tqdm(range(epochs)):
                ...
                for batch, (x, y) in enumerate(dataloader):
                    ...
                    if model == Autoencoder_decoupled:
                        ...
                        optimizer.zero_grad()
                        if params['VAE'] is not None:
                            adjusted_forecast, mu, log_sigma = net(x)
                            loss = criterion(adjusted_forecast, y,mu, log_sigma, mask = m) 
                        else:
            ...
            ##################################################################################################################################
                if params['VAE'] is not None:
                    ds_test = ds_test.mean('ensembles')
                ...
                test_set = XArrayDataset(ds_test, obs_test, extra_predictors= extra_predictors, lead_time=None,lead_time_mask = params['lead_time_mask'], time_features=time_features,  in_memory=False, aligned = True, year_max = year_max)
                ...
                if params['VAE'] is not None:
                    test_results = np.zeros_like(np.broadcast_to(ds_test.values[:,None,...] , (ds_test.shape[0], params['VAE'], *ds_test.shape[1:]) ))
                else:
                ...
                # output_ensemble_size = 1 if params['VAE'] is None else params['VAE']
                # members_result_list = []
                # member_lost_list = []
                # for member in range(output_ensemble_size):                       
                        ...
                        if 'ensembles' in ds_test.dims:  ## PG: If we have large ensembles:
                            ...
                            with torch.no_grad():
                                ...
                                if model == Autoencoder_decoupled:
                                    ...
                                    test_adjusted = net(test_raw) if params['VAE'] is None else net(test_raw)[0]              
                                loss = criterion_test(test_adjusted, test_obs) if params['VAE'] is None else criterion_test(torch.mean(test_adjusted, 0), test_obs[...,0,:].unsqueeze(-2))
                ...
                gc.collect()
                if params['VAE'] is not None:
                    test_results = test_results.reshape((test_results.shape[0], test_results.shape[2], test_results.shape[1], *test_results.shape[3:]))
                ##################################################################################################################################
                if model in [UNet , CNN]:   ## PG: if the output is already a map
                    ...
                    test_results_untransformed = obs_pipeline.inverse_transform(test_results_upsampled.values) ## PG: Check preprocessing.AnomaliesScaler for changes
                    ...
                    # members_result_list.append(result)
                    # member_lost_list.append(test_loss)
                    # if output_ensemble_size > 1:
                    #     print(f'Member {member} saved ....')
                # if output_ensemble_size > 1:
                #     result = xr.concat(members_result_list, dim = 'member').assign_coords(member = np.arange(1, output_ensemble_size + 1))
                #     test_loss = np.concatenate(member_lost_list)
                # print(test_results)
                ##################################################################################################################################
...
if __name__ == "__main__":
    ...
    n_years =  36 # last n years to test consecutively
    lead_years = 5
    n_runs = 1  # number of training runs

    params = {
        "model": Autoencoder,
        "hidden_dims": [[1500, 720, 360, 180, 90], [180, 360, 720, 1500]],
    ...
    out_dir_x  = f'/space/hall5/sitestore/eccc/crd/ccrn/users/rpg002/output/fgco2_ems/SOM-FFN/results/{params["model"].__name__}/run_set_7'
    ...
    if params['lr_scheduler']:
        out_dir = out_dir + '_lr_scheduler_obs_V2023'
        params['start_factor'] = 1.0
        params['end_factor'] = 0.1
        params['total_iters'] = 50

    if params['VAE'] is not None:
        out_dir = out_dir + f'_VAE_{params["VAE"]}'
    
    # Path(out_dir).mkdir(parents=True, exist_ok=True)
    # Path(out_dir + '/Figures').mkdir(parents=True, exist_ok=True)

    # run_training(params, n_years=n_years, lead_years=lead_years, n_runs=n_runs, results_dir=out_dir, numpy_seed=1, torch_seed=1, save = True)
